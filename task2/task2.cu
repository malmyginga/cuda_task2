...
__global__ void cuda_filter_rgb_2img(unsigned char *img_red, unsigned char *img_green, unsigned char *img_blue,
    unsigned char *img_res_red, unsigned char *img_res_green, unsigned char *img_res_blue, 
    int filter_size, int height, int width) {
...
float kernel_apply_2img(unsigned char *device_img_red, unsigned char *device_img_green, unsigned char *device_img_blue, 
...
unsigned char *device_img_res_red, unsigned char *device_img_res_green, unsigned char *device_img_res_blue,
float *filter, float *device_filter) {
...
int main(int argc, char **argv) {
    ...
    if (image_size == 1) {
        ...
        for (int i = 0; i < number_of_images; ++i) {
            ...
            std::string curr_filename = filenames[i];
            std::cout << "filename: " << curr_filename << std::endl;
            std::string img_load_path = dir_path_str + curr_filename;

            img = stbi_load(img_load_path.c_str(), &width, &height, &channels, 0);
            if (img == NULL) {
                printf("Error in loading image\n");
                exit(1);
            }
            ...
            if (flag) {
                ...
                SAFE_CALL(cudaEventRecord(stop));
                SAFE_CALL(cudaEventSynchronize(stop));
                SAFE_CALL(cudaEventElapsedTime(&load_time, start, stop));
            }
            transmit_time += load_time;
            load_time = 0.;
            img_rgb_init(img, img_red, img_green, img_blue, width, height, channels);
            free(img);
            ...
            std::string img_save_path_str = dir_save_path_str + curr_filename;
            save_img(img_res_red, img_res_green, img_res_blue, img_path, width, height, img_save_path_str.c_str());
            flag = 0;
        }
        mem_free(img_red, img_green, img_blue, img_res_red, img_res_green, img_res_blue,
            device_img_red, device_img_green, device_img_blue, device_img_res_red, device_img_res_green, device_img_res_blue,
    ...
    std::cout << "TRANSMIT + KERNEL TIME: " << transmit_time + kernel_time << std::endl;
    return 0;
}